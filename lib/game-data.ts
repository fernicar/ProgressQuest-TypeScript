...
export function toArabic(s: string): number {
  ...
  function _arab(ds: string, dn: number) {
    if (s.indexOf(ds) !== 0) return false;
...
export function LevelUpTime(level: number) {
  return Math.round((20 + Math.pow(1.15, level)) * 60);
}

export function Split(s: string, field: number, separator?: string) {
